...
function renderGeoCharts() {
  ...
  Plotly.newPlot("freedomRankingChart", [{
    type: "bar", x: countries.map(c => c.freedomScore), y: countries.map(c => c.name), orientation: "h",
    text: countries.map(c => `Rank ${c.worldRank}`), textposition: "auto", hovertext: countries.map(c => `${c.name}<br>Score: ${c.freedomScore}<br>World rank: ${c.worldRank}`),
    marker: { color: countries.map(c => c.freedomScore), colorscale: "YlGnBu", line: { color: "white", width: 1 } }
  }], { title: "Economic freedom score and world rank", margin: { t: 48, r: 18, b: 36, l: 96 }, xaxis: { range: [0, 100], title: "Score" }, yaxis: { automargin: true }, paper_bgcolor: "rgba(255,255,255,0)", plot_bgcolor: "rgba(255,255,255,0)", font: { family: "Inter, Arial, sans-serif", color: "#122033" } }, { responsive: true, displayModeBar: false });
}
